LOVELY_INTEGRITY = '1135b00fb4d7c89dde322fcbe5b5bfde9effe87278d00139804587a6832d4896'
...
function create_UIBox_notify_alert(_achievement, _type)
  local _c, _atlas = (_type == "Skill") and G.P_SKILLS[_achievement] or G.P_CENTERS[_achievement],
  ...
  local subtext = _type == 'achievement' and localize(G.F_TROPHIES and 'k_trophy' or 'k_achievement') or
    ...
    _type == 'Skill' and localize('k_skill') or
...
if (card.ability.set == "Alchemical" or card.ability.name == "c_ReduxArcanum_philosopher_stone") and (G.STATE == G.STATES.SMODS_BOOSTER_OPENED or G.STATE == G.STATES.SPECTRAL_PACK) and (card.area == G.pack_cards and G.pack_cards) then
    ...
    if (card.area == G.pack_cards and G.pack_cards) and card.ability.consumeable then
        if G.STATE == G.STATES.SMODS_BOOSTER_OPENED and (SMODS.OPENED_BOOSTER and SMODS.OPENED_BOOSTER.config.center.kind == 'c_chip') then
            return {
                n=G.UIT.ROOT, config = {padding = 0, colour = G.C.CLEAR}, nodes={
  ...
  local sell = nil
  local use = nil
  if card.area and (card.area == G.consumeables) and card.playing_card and not (hit_minor_arcana_suits and hit_minor_arcana_suits[card.config.card.suit]) then
      use = 
  ...
  if card.area and (card.area == G.consumeables) and card.playing_card then
      use = 
      {n=G.UIT.C, config={align = "cr"}, nodes={
  ...
  if card.ability.set == "c_chip" then
      return {
        n=G.UIT.ROOT, config = {padding = 0, colour = G.C.CLEAR}, nodes={
          {n=G.UIT.C, config={padding = 0.15, align = 'cl'}, nodes={
            {n=G.UIT.R, config={align = 'cl'}, nodes={
  ...
  elseif card and card.area and card.area.config.skill_table then
      return {
        n=G.UIT.ROOT, config = {padding = 0, colour = G.C.CLEAR}, nodes={
...
function G.UIDEF.card_focus_ui(card)
  ...
  if card.area == G.shop_jokers and G.shop_jokers then
    if card.ability.consumeable and card.ability.set == "c_chip" then
      base_attach.children.buy_and_use = nil
    end
    base_attach.children.buy = G.UIDEF.card_focus_button{
      card = card, parent = base_attach, type = 'buy',
...
HeyListen.reset_listening("shop_reroll")
  ...
  function create_card_for_shop(area)
     if skill_active("sk_grm_scarce_1") then
          G.GAME.joker_rate = 0
      end
  ...
  function G.UIDEF.card_h_popup(card)
    if card.ability_UIBox_table then
      ...
      if (AUT.card_type == "Skill") and (card.config.center and card.config.center.class) then
      ...
      if AUT.card_type ~= 'Locked' and AUT.card_type ~= 'Undiscovered' then
          ...
          for k, v in pairs(SMODS.Stickers) do
            if card and card.ability and string.find(k, "crv") and card.ability[k] and G.your_collection then
...
if G.GAME.modifiers.no_skip then return end
...
function create_UIBox_blind_choice(type, run_info)
  ...
  if type == 'Small' then
    if not G.GAME.superboss_active then
      ...
      if not G.GAME.blind_edition or not G.GAME.blind_edition[type] or not G.P_BLIND_EDITIONS[G.GAME.blind_edition[type]].no_skip then
        ...
        {n=G.UIT.R, config={align = "cm", padding = 0.05}, nodes={
          {n=G.UIT.R, config={id = 'blind_desc', align = "cm", padding = 0.05}, nodes={
            ...
            {n=G.UIT.R, config={align = "cm",r = 0.1, padding = 0.05, minw = 3.1, colour = G.C.BLACK, emboss = 0.05}, nodes={
              ((type == "Boss") and G.GAME.nullified_blinds[G.GAME.round_resets.blind_choices["Boss"]]) and {n=G.UIT.R, config={align = "cm", maxw = 3}, nodes={
              ...
              _reward and {n=G.UIT.R, config={align = "cm"}, nodes={
                ...
                {n=G.UIT.T, config={text = dollars_to_string(extra_dollars_mod or blind_choice.config.dollars)..'+', scale = 0.35, colour = disabled and G.C.UI.TEXT_INACTIVE or (((extra_dollars_mod or blind_choice.config.dollars) < 0) and G.C.RED or G.C.MONEY), shadow = not disabled}}
              }} or nil,
            }},
          }},
        }},
...
function create_UIBox_round_scores_row(score, text_colour)
  ...
  if score == 'defeated_by' and G.GAME.akyrs_defeated_by_center then 
      score_tab, label = AKYRS.defeated_by_center_ui(G.GAME.akyrs_defeated_by_center)
  else
...
function create_UIBox_hand_tip(handname)
  ...
  for k, v in ipairs(G.GAME.hands[handname].example) do
      ...
      if v['akyrs_letter'] then card.ability.forced_letter_render = true card:set_letters(v['akyrs_letter']) end
...
function create_UIBox_blind_popup(blind, discovered, vars)
  ...
  if discovered then 
    ...
    AKYRS.getBlindText(blind.key, "collection")[1] and
    {n=G.UIT.R, config={align = "cm", emboss = 0.05, r = 0.1, minw = 2.5, padding = 0.07, colour = G.C.WHITE}, nodes={
        {n=G.UIT.R, config={align = "cm", maxw = 2.4}, nodes={
            {n=G.UIT.T, config={text = AKYRS.getBlindText(blind.key, "collection")[1], scale = 0.35, colour = G.C.UI.TEXT_DARK}},
        }},
    {n=G.UIT.R, config={align = "cm"}, nodes={
        {n=G.UIT.O, config={object = stake_sprite}},
        AKYRS.mod_blind_display(blind) or
        {n=G.UIT.T, config={text = AKYRS.getBlindText(blind.key, "collection")[2], scale = 0.4, colour = G.C.RED}},
    }},
    {n=G.UIT.R, config={align = "cm"}, nodes={
        {n=G.UIT.T, config={text = localize('ph_blind_reward'), scale = 0.35, colour = G.C.UI.TEXT_DARK}},
        {n=G.UIT.O, config={object = DynaText({string = {dollars_to_string(_dollars)}, colours = {(_dollars < 0) and G.C.RED or G.C.MONEY}, rotate = true, bump = true, silent = true, scale = 0.45})}},
    }},
...
function G.UIDEF.stake_option(_type)
  ...
  if G.bmbacks_key[G.GAME.viewed_back.effect.center.key] then max_stake = #G.P_CENTER_POOLS['Stake'] end
...
function G.UIDEF.challenge_description(_id, daily, is_row, is_hardcore)
  ...
  local joker_col = {n=G.UIT.C, config={align = "cm", padding = 0.05, colour = G.C.L_BLACK, r = 0.1, maxh = 1.8}, nodes={
    ...
    {n=G.UIT.C, config={align = "cm", minh = 0.6*G.CARD_H, minw = 4.4, r = 0.1, colour = G.C.UI.TRANSPARENT_DARK}, nodes={
...
function G.UIDEF.challenge_description_tab(args)
  ...
  if args._tab == 'Rules' then
    ...
    local base_modifiers = {
      ...
      force_stake_xp = {value = 100, order = 8},
  }
