LOVELY_INTEGRITY = '6d6d4cdc136285ec2f6da6f6eb7f5a4e11bfef121530fc2d48d906733c836d8f'
...
function set_consumeable_usage(card)
    if card.config.center_key and card.ability.consumeable then
      ...
      G.GAME.consumeable_usage_total.akyrs_replicant = G.GAME.consumeable_usage_total.akyrs_replicant or 0
      if card.config.center.set == 'Replicant' then 
              G.GAME.consumeable_usage_total = G.GAME.consumeable_usage_total or {}
              G.GAME.consumeable_usage_total.akyrs_replicant = (G.GAME.consumeable_usage_total.akyrs_replicant or 0) + 1
              G.E_MANAGER:add_event(Event({
                      trigger = 'immediate',
                      func = function()
                      G.E_MANAGER:add_event(Event({
                      trigger = 'immediate',
                      func = function()
                      G.GAME.akyrs_last_replicant = card.config.center_key
                              return true
                      end
                      }))
                      return true
...
function loc_colour(_c, _default)
  ...
  G.ARGS.LOC_COLOURS.akyrs_replicant_o = G.C.AKYRS_REPLICANT_O
  G.ARGS.LOC_COLOURS.akyrs_replicant_c = G.C.AKYRS_REPLICANT_C
