LOVELY_INTEGRITY = '581bf6a98260e9ed8bcc85e55fb8b5e5c4a059570cb5cc764c6838120242d338'
...
G.FUNCS.HUD_blind_reward = function(e)
  if (G.GAME.modifiers.no_blind_reward and (G.GAME.blind and G.GAME.modifiers.no_blind_reward[G.GAME.blind:get_type()])) and (not G.GAME.blind_edition or not G.GAME.blind_edition[G.GAME.blind:get_type()] or not G.P_BLIND_EDITIONS[G.GAME.blind_edition[G.GAME.blind_on_deck]].dollars_mod or 
...
G.FUNCS.overlay_menu  = function(args)
  ...
  if Handy.regular_keybinds.swappable_overlay then
...
G.FUNCS.chip_UI_set = function(e)
  ...
  if G.AKYRS_DISPLAY_QUEUE and G.AKYRS_DISPLAY_QUEUE[1] then 
...
G.FUNCS.hand_text_UI_set = function(e)
  ...
  G.FUNCS.can_sell_card = function(e)
    if e.config.ref_table:can_sell_card() then 
        ...
        if e.config.ref_table.ability.eternal and skill_active("sk_grm_sticky_1") then
...
(G.STATE == G.STATES.SMODS_BOOSTER_OPENED or G.STATE == G.STATES.PLANET_PACK or G.STATE == G.STATES.STANDARD_PACK or G.STATE == G.STATES.BUFFOON_PACK or (G.hand  )) then 
  ...
  G.FUNCS.show_infotip = function(e)
    ...
    if G.round_eval and not G.round_eval.alignment.offset.py and not PLANET_CARD_CASH_OUT_MOD_COND then
    ...
    if card and skill_active("sk_mf_painted_3") and card.ability and (card.ability.set == "Colour") and not card.ability.grm_already_used then
    ...
    if card and skill_active("sk_mf_painted_3") and card.ability and (card.ability.set == "Colour") and activate_nc then
        activate_nc = nil
        nc = true
    end
    ...
    elseif card.ability.set == 'Enhanced' or card.ability.set == 'Default' or card.ability.set == 'Perk' then
    ...
    elseif card.ability.set == 'Skill' then
        learn_skill(card)
...
G.FUNCS.buy_from_shop = function(e)
    ...
    if c1.ability.disguised then
        card_transform(c1, c1.ability.disguised)
  ...
  if G.GAME and G.GAME.modifiers and (G.GAME.modifiers.ccc_bside and G.GAME.modifiers.ccc_bside >= 1) then
    ...
    if _tag and _tag.config.ref_table then
        local valid = false
  ...
  G.FUNCS.reroll_boss_button = function(e)
    ...
    if ((G.GAME.dollars + (G.GAME.virtual_dollars or 0) - G.GAME.bankrupt_at) - reroll_cost >= to_big(0)) and
  ...
  if HeyListen.on_shop_reroll(e) then return end
    ...
    G.E_MANAGER:add_event(Event({
        ...
        func = function()
          ...
          G.GAME.grm_unlocks.shop_rerolls = G.GAME.grm_unlocks.shop_rerolls + 1
...
if Handy.insta_cash_out.is_skipped and e.config.button then return end
    stop_use()
      if G.round_eval then  
        ...
        if to_big and skill_active("sk_grm_receipt_1") and (to_big(G.GAME.current_round.dollars) >= to_big(4)) then
