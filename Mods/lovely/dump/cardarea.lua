LOVELY_INTEGRITY = '8fd16a4f51e33b1203e7918d44dc8a043b35c4b47bf2b8f808686131654aa2de'
...
function CardArea:add_to_highlighted(card, silent)
   if (self == G.hand) and G.GAME and G.GAME.modifiers and G.GAME.modifiers.dungeon then
    ...
    elseif self.config.type == 'joker' or self.config.type == 'consumeable' then
        ...
        if #self.highlighted >= self.config.highlighted_limit or SMODS.has_enhancement(card, "m_reverse_iridium") then
...
function CardArea:align_cards()
    ...
    if self.config.type == 'hand' and (G.STATE == G.STATES.TAROT_PACK or G.STATE == G.STATES.SPECTRAL_PACK or G.STATE == G.STATES.PLANET_PACK or G.STATE == G.STATES.SMODS_BOOSTER_OPENED or G.STATE == G.STATES.STANDARD_PACK or G.STATE == G.STATES.BUFFOON_PACK) then
    ...
    if self.config.type == 'hand' and not (G.STATE == G.STATES.TAROT_PACK or G.STATE == G.STATES.SPECTRAL_PACK or G.STATE == G.STATES.PLANET_PACK or G.STATE == G.STATES.SMODS_BOOSTER_OPENED or G.STATE == G.STATES.STANDARD_PACK or G.STATE == G.STATES.BUFFOON_PACK) then
        local max_cards_override = (Cartomancer.SETTINGS.dynamic_hand_align and self.config.temp_limit - #self.cards > 5) and math.max(#self.cards, math.min(10, self.config.temp_limit))
