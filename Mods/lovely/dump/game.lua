LOVELY_INTEGRITY = '59628bb9db99d8420cdbd8e18860660c704bc9cd5dc21de3990a0a8da3cf55dd'
...
function skill_meets_dependencies(key)
...
function Game:start_up()
    ...
    if not AKYRS then
        AKYRS_LOAD_FAIL = true
    end
    assert(not AKYRS_LOAD_FAIL,[[ 
    (X^X)
    
    Hey! If you're reading this message, that means Aikoyori's Shenanigans has crashed due to outdated dependencies :<
    (In simple words, your Steamodded and/or Lovely Injector that you have is too old for the mod)
    
    Update your **Steamodded** and **Lovely Injector** to latest stable version.
    
    Generally, every major update of the mod bumps the requirements to the latest available stable version of these.
    
    To Update Steamodded, download AND replace your existing Steamodded folder with a new one
    
    To Update Lovely Injector, replace the `version.dll` in your installation (i.e. where you open the game) with a newer version
    >>> LOVELY INJECTOR IS NOT SUPPOSED TO BE IN THE MODS FOLDER. IT SHOULD BE NEXT THE BALATRO.EXE <<<
    ...
    local a = (#self.P_CENTER_POOLS['Skill'] == 0)
...
function Game:main_menu(change_context) --True if main menu is accessed from the splash screen, false if it is skipped or accessed from the game
    ...
    G.SPLASH_BACK:define_draw_steps({{
        ...
        send = {
            ...
            {name = 'vort_speed', val = 0.4},
                        {name = 'colour_1', ref_table = G.C, ref_value = 'AKYRS_AIKOYORI_BOW'},
                        {name = 'colour_2', ref_table = G.C, ref_value = 'AKYRS_AIKOYORI_HAIR'},
...
G.SPLASH_LOGO = Sprite(0, 0, 
  ...
  G.ASSET_ATLAS[key], {x=0,y=0})
    ...
    SMODS.Stickers['ns_yhtnsmod']:apply(replace_card, true)

    replace_card:set_seal('Gold', true, true)
    rh_beat_anim(replace_card)
...
function Game:init_game_object()
    ...
    return {
        ...
        chips = 0,
        chips_text = '0',
        base_ante_scaling = get_starting_params().ante_scaling,
        scaling_multipliers = {},
        skills = {},
        skill_xp = 0,
        xp_spent = 0,
...
if args and args.savetext then
  BPlus.apply_metatable_to_probabilities(args.savetext.GAME)
    ...
    self.GAME.cere = self.GAME.cere or {jokers = {}, hand_size = 0, shredded ={}, can_hit = true, can_stand = true}
    if Talisman and Talisman.igo then self.GAME = Talisman.igo(self.GAME) end
    Handy.UI.init()
    ...
    if not saveTable then 
        ...
        G.GAME.modifiers.force_stake_xp = self.GAME.starting_params.force_stake_xp
    ...
    self.cere_perks = not Ceres.COMPAT.loyaltycard and CardArea(
            0, 0,
            CAI.hand_W,CAI.hand_H, 
            {card_limit = self.GAME.starting_params.hand_size, type = 'hand'})
    self.cere_exploit = not Ceres.COMPAT.loyaltycard and CardArea(
            0, 0,
            CAI.play_W,CAI.play_H, 
            {card_limit = 1, type = 'play'})
    self.cere_burnt = not Ceres.COMPAT.loyaltycard and CardArea(
            0, 0,
            CAI.discard_W,CAI.discard_H,
            {card_limit = 500, type = 'discard'})
    self.deck = CardArea(
    ...
    G.garb_kirby_W = 4.9*G.CARD_W
    G.garb_kirby_H = 0.95*G.CARD_H
    
    G.garb_kirby = CardArea(
        G.jokers.T.x,
        G.jokers.T.y - (G.jokers.T.h*2), 
        G.garb_kirby_W, 
        G.garb_kirby_H, 
        {card_limit = 1, type = 'cs_stack', highlight_limit = 0}
    )
    
    G.garb_void_W = 4.9*G.CARD_W
    G.garb_void_H = 0.95*G.CARD_H
    
    G.garb_void = CardArea(
        G.jokers.T.x,
        G.jokers.T.y - (G.jokers.T.h*2), 
        G.garb_void_W, 
        G.garb_void_H, 
    ...
    if saveTable then 
        ...
        if not card_protos then 
            ...
            for k, v in pairs(self.P_CARDS) do
                ...
                if self.GAME.starting_params.one_suit and not v.suit == self.GAME.starting_params.one_suit then keep = false end
                if _de then
...
function Game:update(dt)
    ...
    if G and G.GAME and G.GAME.modifiers and G.GAME.modifiers.dungeon and G.hand and G.hand.highlighted then
        ...
        if self.STATE == self.STATES.SELECTING_HAND then
            if (not G.hand.cards[1]) and G.deck.cards[1] and not G.GAME.cere then
        ...
        (not G.FILE_HANDLER.last_sent_time or (G.FILE_HANDLER.last_sent_time < (G.TIMERS.UPTIME - G.F_SAVE_TIMER)))) then 
            ...
            elseif G.FILE_HANDLER.settings then
                G.SAVE_MANAGER.channel:push({
                    ...
                    profile_num = G.SETTINGS.profile,
                    save_profile = Talisman.sanitize(Talisman.copy_table(G.PROFILES[G.SETTINGS.profile]))
                  })
            end

            if G.FILE_HANDLER.run then
            Talisman.sanitize(G.ARGS.save_run)
                G.SAVE_MANAGER.channel:push({
                    type = 'save_run',
...
love.graphics.pop()
    ...
    if G.SETTINGS.Enhance ~= nil and  G.SETTINGS.Enhance.showFPS then
        love.graphics.push()
                    ...
                    G.E_MANAGER:add_event(Event({
                        ...
                        func = function()
                            ...
                            Handy.regular_keybinds.on_shop_loaded()
                                ...
                                local old_shop_exists = shop_exists
                                ...
                                if not shop_exists then
                                    ...
                                    if G.load_shop_jokers then 
                                        ...
                                        if G.GAME.modifiers.alchemy_deck then
                                        	local opus = create_card("Myth",G.shop_jokers,nil,nil,false,false,"c_prism_myth_opus")
