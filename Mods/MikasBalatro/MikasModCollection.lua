...
--- MOD_DESCRIPTION: A collection of Mika's Mods. Check the mod description on GitHub for more information :). Remix by Metaphysical_Pizza.
...
function SMODS.INIT.MikasModCollection()
    ...
    if config.showoffJoker then
        ...
        SMODS.Jokers.j_mmc_showoff.calculate = function(self, context)
            ...
            if context.end_of_round and not context.individual and not context.repetition and not context.blueprint then
                if G.GAME.chips >= 2 * G.GAME.blind.chips then
                    self.ability.extra.current_Xmult = self.ability.extra.current_Xmult + self.ability.extra.Xmult_mod
    ...
    if config.grudgefulJoker then
        ...
        local grudgeful = {
            loc = {
                ...
                text = {
                    "每回合选择盲注时",
                    "以上一回合{C:attention}得分超额#2#%{}以上的部分",
                    "削减当前盲注的最低得分要求",
        ...
        function SMODS.Jokers.j_mmc_grudgeful.loc_def(card)
            return { number_format(card.ability.extra.old_chips), card.ability.extra.percentage, card.ability.extra.a, vars = {card.ability.extra.current_chips, card.ability.extra.total_chips} }
        end
        ...
        SMODS.Jokers.j_mmc_grudgeful.calculate = function(self, context)
            ...
            if context.end_of_round and not context.individual and not context.repetition and not context.blueprint then
                if self.ability.extra.total_chips >= G.GAME.blind.chips * (1 + self.ability.extra.percentage / 100) then
                    self.ability.extra.current_chips = math.floor(self.ability.extra.total_chips - G.GAME.blind.chips * (1 + self.ability.extra.percentage / 100), 0)
                ...
                self.ability.extra.total_chips = 0
                self.ability.extra.a = self.ability.extra.a + 1
            end
            if context.setting_blind and not context.blueprint then
                if self.ability.extra.a > 0 then
                    if (SMODS.Mods["Talisman"] or {}).can_load then
                        if to_number(G.GAME.blind.chips - self.ability.extra.current_chips) > 0 then
                            G.GAME.blind.chips = math.floor(G.GAME.blind.chips - self.ability.extra.current_chips, 0)
                        ...
                        if  G.GAME.blind.chips - self.ability.extra.current_chips > 0 then
                            G.GAME.blind.chips = math.floor(G.GAME.blind.chips - self.ability.extra.current_chips, 0)
                            G.GAME.blind.chip_text = number_format(G.GAME.blind.chips)
                        else
                            G.GAME.blind.chips = 0
                    ...
                    self.ability.extra.a = 1
                    card_eval_status_text(self, "extra", nil, nil, nil, {
                        message = localize("k_reset")
                    })
                end
                -- Reset total chip count
                self.ability.extra.old_chips = 0
