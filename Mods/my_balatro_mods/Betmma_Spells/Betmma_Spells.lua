...
function SMODS.current_mod.process_loc_text()
    ...
    local dict = G.localization.misc and G.localization.misc.dictionary
    if dict then
        local function set_default(key, value)
            if dict[key] == nil then dict[key] = value end
        end
        set_default('b_fuse', "FUSE")
        set_default('b_reroll', "REROLL")
        set_default('spell_not', "not ")
        set_default('spell_Heart', "Heart")
        set_default('spell_Diamond', "Diamond")
        set_default('spell_Club', "Club")
        set_default('spell_Spade', "Spade")
        set_default('spell_Ace', "Ace")
        set_default('spell_Face', "Face")
        set_default('spell_Numbered', "Numbered")
        set_default('k_betmma_spell_pack', "Spell Pack")
        set_default('k_betmma_smoke', "Smoke!")
        set_default('k_betmma_downgrade_ex', "Downgrade!")
        set_default('k_betmma_reflection', "Reflection!")
        set_default('k_betmma_crystal', "Crystalization!")
    end

    local other = G.localization.descriptions and G.localization.descriptions.Other
    if other then
        other.card_extra_mult = other.card_extra_mult or { text = { "{C:mult}+#1#{} extra mult" } }
        other.card_extra_p_dollars = other.card_extra_p_dollars or { text = { "{C:money}+$#1#{}" } }
        other.card_extra_mult_neg = other.card_extra_mult_neg or { text = { "{C:red}#1#{} extra mult" } }
        other.card_extra_p_dollars_neg = other.card_extra_p_dollars_neg or { text = { "{C:red}$#1#{}" } }
    end

    -- Ensure Spell descriptions keep the sequence/progress lines regardless of active language
    local spell_desc = G.localization.descriptions and G.localization.descriptions.Spell
    if spell_desc then
        local SEQ_LINE = "{V:1}#11#{}{V:5}#15#{}{V:2}#12#{}{V:6}#16#{}{V:3}#13#{}{V:7}#17#{}{V:4}#14#{}"
        local PROGRESS_LINE = "{C:attention}#18#{}/{C:attention}#19#{}"
        local FUSED_LINE = "{C:inactive,s:0.8}(#21# + #22#){}"
        local fused_keys = {}
        if betmma_spells_centers then
            for _, center in pairs(betmma_spells_centers) do
                if center and center.key and center.config and center.config.fuse_from then
                    fused_keys[center.key] = true
                end
            end
        end
        local function list_has(texts, needle)
            if type(texts) ~= 'table' then return false end
            for i = 1, #texts do
                local line = texts[i]
        ...
        for key, entry in pairs(spell_desc) do
            if type(key) == 'string' and key:find('c_betm_spells_', 1, true) == 1 and type(entry) == 'table' then
                if type(entry.text) == 'table' then
                    if not (type(entry.text[1]) == 'string' and entry.text[1]:find('#11#', 1, true)) then
                        table.insert(entry.text, 1, SEQ_LINE)
                    end
                    if not list_has(entry.text, '#18#') then
                        entry.text[#entry.text + 1] = PROGRESS_LINE
                    end
                    if fused_keys[key] and not list_has(entry.text, '#21#') then
                        entry.text[#entry.text + 1] = FUSED_LINE
                    end
                end
            end
        end
...
local function spell_prototype(data)
    ...
    data.loc_vars = function(self, info_queue, card)
        ...
        local function get_center_name(center)
            if not center then return "" end
            local ok, name = pcall(function()
                return localize{type = 'name_text', set = center.set, key = center.key}
            end)
            if ok and name and name ~= "ERROR" then return name end
            return (center.loc_txt and center.loc_txt.name) or ""
        end
        local function get_before_desc(center)
            if not center then return "" end
            local set = center.set
            local key = center.key
            local desc = (G.localization and G.localization.descriptions and set and key and G.localization.descriptions[set] and G.localization.descriptions[set][key]) or nil
            return (desc and desc.before_desc) or (center.loc_txt and center.loc_txt.before_desc) or ""
        end
        if fused then
            ret.vars[21]=get_center_name(betmma_spells_centers and betmma_spells_centers[fused[1]] or nil)
            ret.vars[22]=get_center_name(betmma_spells_centers and betmma_spells_centers[fused[2]] or nil)
        end
        if not card.ability.progress.max then
            ret.vars[11]=get_before_desc(card.config.center)
            return ret
        end
