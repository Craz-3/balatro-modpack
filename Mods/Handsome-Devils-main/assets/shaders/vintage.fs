...
// Look vintage.fs for explanation
...
vec2 centered_uv = uv - vec2(0.35, 0.35);
tex = vec4(tex.rgb + smoothstep(0., 0.99, centered_uv.x * centered_uv.x + centered_uv.y * centered_uv.y), tex.a);



vec3 vintageVec = tex.rgb;
if(vintage.g > 0.0 || vintage.g < 0.0) {
vintageVec.r = (tex.r * 0.293) + (tex.g * 0.669) + (tex.b * 0.139);
vintageVec.g = (tex.r * 0.249) + (tex.g * 0.586) + (tex.b * 0.118);
vintageVec.b = (tex.r * 0.172) + (tex.g * 0.434) + (tex.b * 0.081);
}

tex = vec4(vintageVec, tex.a);
